...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $CardID = isset($_POST['CardID']) ? $_POST['CardID'] : null;  // Optional
    $TransactionType = $_POST ['TransactionType'];
    $Amount = $_POST['Amount'];
    $TransactionDate = $_POST ['TransactionDate'];
    $Remarks = $_POST ['Remarks'];

    // Prepare the SQL statement with corrected field name
    $sql = "INSERT INTO transactions (TransactionID, AccountID, CardID, TransactionType, Amount, TransactionDate, Remarks) 
            VALUES (?, ?, ?, ?, ?, ?, ?)";

    // Use prepared statements to prevent SQL injection
    $stmt = $conn->prepare($sql);

    // Correct bind_param to match data types and number of parameters
    $stmt->bind_param("iisssss", $TransactionID, $AccountID, $CardID, $TransactionType, $Amount, $TransactionDate, $Remarks);
    ...
    if ($stmt->execute()) {
        ...
        echo '<div style="padding: 15px; margin: 15px 0; background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; border-radius: 8px; display: flex; align-items: center; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                ...
                <strong>Success!</strong> New Transaction added successfully.
              </div>';
    } else {
