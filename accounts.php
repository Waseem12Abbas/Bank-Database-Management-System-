...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // SQL query to insert data into accounts table using prepared statements
    $sql = "INSERT INTO accounts (AccountID, AccountNumber, AccountType, Balance, BranchID, CreatedAt) 
            VALUES (?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("issdss", $accountID, $accountNumber, $accountType, $balance, $branchID, $createdAt);
    ...
    if ($stmt->execute()) {
        ...
        echo '<div style="padding: 15px; margin: 15px 0; background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; border-radius: 8px; display: flex; align-items: center; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                ...
                <strong>Success!</strong> New account added successfully.
              </div>';
    } else {
