...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Collecting form data safely using prepared statements
    $cardID = $_POST['cardID'];
    ...
    // SQL query to insert data into credit_cards table using prepared statements
    $sql = "INSERT INTO CreditCards (CardID, CustomerID, CardNumber, ExpiryDate, CVV, CreditLimit, Balance, Status) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("iisssdss", $cardID, $customerID, $cardNumber, $expiryDate, $cvv, $creditLimit, $balance, $status);
    ...
    if ($stmt->execute()) {
        ...
        echo '<div style="padding: 15px; margin: 15px 0; background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; border-radius: 8px; display: flex; align-items: center; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                ...
                <strong>Success!</strong> New creditcard added successfully.
              </div>';
    } else {
